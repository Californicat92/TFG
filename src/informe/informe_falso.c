...
{
	sqlite3 *db;		//Puntero a base de datos
	char sql[120];
	char *zErrMsg = 0;	//msg error
	int rc;				//aqui se va a retornar la sentencia
	int nSen;
	int nAlarm;
	char data[200];
	char texto[400];
	...
	**/
	/* Open database */
	rc = sqlite3_open("captura.db", &db);	//Se abre puente con la bbdd
	...
	rc = sqlite3_exec(db, sql, getValues, (void *)data, &zErrMsg);	//insertamos datos en la tabla
	...
	for (n = 1; n <= nSen; n++) {
		...
		// Realizamos consultas sobre diferentes campos en la tabla bbdd
		
		//Fecha/hora inicial
		sprintf(sql, "SELECT MIN(Date_time_lecture)"\
			" FROM Lectures_table WHERE ID = %d", n);
		rc = sqlite3_exec(db, sql, getValues, (void *)data, &zErrMsg);	//seleccionamos datos de la tabla bbdd 
		fecha = strtok(data, " ");
		hora = strtok(hora, " ");
