...
int main(int argc, char* argv[]) {
	...
	float value_volts;
		...
		char on[] = 		"RCPT TO: 1422047@campus.euss.org \n";
		char data[] = 		"DATA \n";
		char subject[] =	"Subject: primer mail programat en c \n" \
							...
							"Bon dia, \n " \
							"S'ha superat el nivell de tensió esperat.\n " \
							"Salutacions, \n " \
							"Christopher Juvé " \
							"\n.\n";
		char adeu[] = 		"quit";	
	...
	while(1){
		...
		if(value_volts > VMAX){
			...
			//1	
				...
				/*Enviar*/
				strcpy(buffer,qui_soc); //Copiar missatge a buffer
			...
			//2
				...
				/*Enviar*/
				strcpy(buffer,desde); //Copiar missatge a buffer
				result = write(sFd, buffer, strlen(buffer));
				printf("Missatge enviat a servidor(bytes %d): %s\n",	result, buffer);
			//3
				/*Rebre*/
				...
				/*Enviar*/
				strcpy(buffer,on); //Copiar missatge a buffer
				result = write(sFd, buffer, strlen(buffer));
				printf("Missatge enviat a servidor(bytes %d): %s\n",	result,buffer );
				/*Tancar el socket*/
			//4
			...
			//6
				...
				/*Enviar*/
				strcpy(buffer,adeu); //Copiar missatge a buffer
				result = write(sFd, buffer, strlen(buffer));
				printf("Missatge enviat a servidor(bytes %d): %s\n",	result, buffer);
				/*Tancar el socket*/
